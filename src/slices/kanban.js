...
export const getBoardProject = (caseId) => async (dispatch) => {
    const data = await kanbanApi.getBoard(null, caseId);
...
export const updateCard = (cardId, update) => async (dispatch) => { // DONE: connect to server
...
export const addComment = (cardId, message, userId) => async (dispatch) => {
  const data = await kanbanApi.addComment({ cardId, message, userId });
